...
<head>
    ...
    <script type="text/javascript" href="../includes/jquery.js"> </script>
    ...
    <section>
        ...
        <table class="table_displayData" style="margin-top:20px;">
            ...
            <tr>
                ...
                <td>
                    <?php
                        if($row_selectOrder['approved'] == 1) {
                            // Check if invoice exists
                            $order_id = $row_selectOrder['order_id'];
                            $query_invoice = "SELECT * FROM invoice WHERE order_id = '$order_id'";
                            $result_invoice = mysqli_query($con, $query_invoice);
                            
                            if(mysqli_num_rows($result_invoice) > 0) {
                                // Invoice exists - show view link
                                echo "<a href=\"view_invoice.php?id=".$order_id."\"></a>";
                                
                                // Update status to completed if not already
                                if($row_selectOrder['status'] == 0) {
                                    $query_update = "UPDATE orders SET status = 1 WHERE order_id = '$order_id'";
                                    mysqli_query($con, $query_update);
                                }
                            } else {
                                // No invoice exists - show generate link if status is pending
                                if($row_selectOrder['status'] == 0) {
                                    echo "<a href=\"generate_invoice.php?id=".$order_id."\">+ Invoice</a>";
                                } else {
                                    echo "N/A";
                                }
                            }
